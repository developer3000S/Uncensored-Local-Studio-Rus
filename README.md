...
<details>
  <summary><strong>Ошибка эмбеддинга: «input is too large to process»</strong></summary>
  <p>Эта ошибка возникает когда текст, переданный в эмбеддинг-модель, превышает физический размер батча сервера llama.cpp (по умолчанию 512 токенов). Начиная с текущей версии, запросы автоматически обрезаются до ~1500 символов перед отправкой в эмбеддинг, а чанки RAG нарезаются по ~400 символов, что безопасно укладывается в этот лимит. Если ошибка всё равно появляется, убедитесь, что используется актуальная версия <code>scripts/server/serve.cjs</code>.</p>
</details>
