...
* [Внесение изменений & коммиты](#внесение-изменений--коммиты)
...
3. **Генерация:** Откройте `http://localhost:14200` в браузере, выберите модель и введите подсказку.
...
4. **Генерация:** открыть `http://localhost:14200`.
...
<details>
  ...
  <p>UI использует порт 14200, бекенд – 8080. При конфликте скрипт автоматически ищет свободный порт в диапазоне 14201‑14999.</p>
</details>
...
Для автоматизации коммитов и обновления репозитория используется скрипт `update.sh`.

### Использование
```bash
./update.sh "Ваше сообщение коммита"
```

### Что делает скрипт:
1. Добавляет директорию `/disk/llm` в безопасные директории Git (решает проблему с dubious ownership)
2. Добавляет все изменения в индекс Git
3. Создаёт коммит с указанным сообщением
4. Отправляет изменения в ветку `main` на GitHub
5. Устанавливает зависимости npm и собирает frontend приложение (Vite + React)

### Требования
- Убедитесь, что у вас есть права на запись в репозиторий
- Node.js и npm должны быть установлены для сборки frontend
- Git должен быть настроен с вашими учётными данными
