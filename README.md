...
* **Чат с агентом**: Клик по агенту в боковой панели мгновенно открывает вкладку с чат-отладкой для общения с выбранным ИИ-помощником. Чат полностью синхронизирован по функционалу с основным Текстовым чатом: поддерживает прикрепление файлов (изображений и документов с контекстным считыванием), Web-поиск в реальном времени с выводом источников, Deep Thinking (режим глубокого рассуждения) для думающих моделей, потоковую (streaming) генерацию ответов, отображение рассуждений модели с секундомером, форматирование Markdown и возможность принудительной остановки вывода.
* **Постоянная память и обучение**: Все разговоры с агентом автоматически сохраняются в базу данных (`agent_chats`). Агент имеет постоянный доступ к истории сообщений (последние 20 сообщений используются как скользящее контекстное окно) как в режиме чата, так и при выполнении фоновых задач. Вы также можете очистить историю в любой момент кнопкой «Очистить историю».
* **Кастомные настройки**: Каждый агент имеет имя, описание и системный промпт, определяющий его характер и поведение.
* **Локальный RAG (База знаний)**: Загружайте файлы в форматах `.pdf`, `.txt`, `.md`. Система автоматически разобьет их на текстовые сегменты, сгенерирует векторные эмбеддинги и сохранит в локальную базу данных.
* **Личный или Общий доступ**: Вы можете ограничить доступ к файлам только для одного агента («Личный RAG») или сделать их доступными для всех агентов («Общий RAG»).
* **Фоновые задачи**: Вы можете отправить агенту задание для выполнения в фоновом режиме. Задачи выполняются строго по очереди (FIFO) для предотвращения перегрузки видеопамяти (VRAM). Вы можете просматривать интерактивную консоль логов выполнения каждой задачи в реальном времени.

База данных, векторные индексы эмбеддингов, история переписки агентов и фоновые задачи хранятся локально в реляционной базе данных `app/config/studio.db` (SQLite). Данные векторизуются локальными моделями для обеспечения конфиденциальности.
